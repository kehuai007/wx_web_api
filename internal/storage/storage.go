...
func (s *Storage) Init(path string) error {
	...
	// Migrate older DBs that predate client_ip / token_value. CREATE TABLE
	// IF NOT EXISTS won't add columns to an existing table, so we probe via
	// PRAGMA table_info and ALTER if absent. Safe to run repeatedly: the
	// probe returns the column once migration has run.
	if err := ensureClientIPColumn(db); err != nil {
		return fmt.Errorf("migrate client_ip: %w", err)
	}
	if err := ensureTokenValueColumn(db); err != nil {
		return fmt.Errorf("migrate token_value: %w", err)
	}
...
// ensureTokenValueColumn is the token_value counterpart of ensureClientIPColumn:
// backfills the column for databases that predate the masked-token log field.
func ensureTokenValueColumn(db *sql.DB) error {
	rows, err := db.Query("PRAGMA table_info(request_log)")
	if err != nil {
		return fmt.Errorf("pragma table_info: %w", err)
	}
	...
	if _, err := db.Exec(addColumnTokenValue); err != nil {
		return fmt.Errorf("alter add token_value: %w", err)
	}
	return nil
}
...
func (s *Storage) LogRequest(r *RequestLog) error {
	...
	res, err := s.db.Exec(
		`INSERT INTO request_log (ts, token_label, token_value, kind, source, client_ip, request, status, latency_ms, msg, result_data)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		r.Ts, r.TokenLabel, r.TokenValue, r.Kind, r.Source, r.ClientIP, string(r.Request), r.Status, r.LatencyMs, r.Msg, nullableJSON(r.Result),
	)
	if err != nil {
...
func (s *Storage) QueryHistory(q HistoryQuery) (*HistoryPage, error) {
	...
	// page
	listSQL := "SELECT id, ts, token_label, token_value, kind, source, client_ip, request, status, latency_ms, msg, result_data FROM request_log" +
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.Ts, &r.TokenLabel, &r.TokenValue, &r.Kind, &r.Source, &r.ClientIP,
