...
// Sets "token_label", "token_value_masked", and "source" on the gin context
// for downstream handlers and for the writeLog helper to pick up. On any 401,
// an async log row is written with kind='auth' so admins can audit rejected
// calls — the masked token the client attempted is preserved either way.
func (h *Handler) TokenAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Mask the token the client sent (if any) so we can log it on 401 paths.
		// Empty string when no header/query was provided.
		masked := maskToken(token)
		if token == "" {
			h.writeLog(c, t0, "", masked, "external", "auth", 401, "missing token", nil, gin.H{"path": c.Request.Method + " " + c.FullPath()})
		...
		if matchedTok == nil {
			h.writeLog(c, t0, "", masked, "external", "auth", 401, "unknown token", nil, gin.H{"path": c.Request.Method + " " + c.FullPath()})
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"code": 401, "msg": "unauthorized"})
			return
		}
		if h.isExpired(matchedTok.ExpiresAt) {
			h.writeLog(c, t0, matchedTok.Label, masked, "external", "auth", 401, "token expired", nil, gin.H{"path": c.Request.Method + " " + c.FullPath()})
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"code": 401, "msg": "token expired"})
			return
		}

		// Auth ok — inject label, masked value, and source for downstream handlers.
		source := c.GetHeader("X-Wx-Source")
		if source != "admin_test" {
			source = "external"
		}
		c.Set("token_label", matchedTok.Label)
		c.Set("token_value_masked", masked)
		c.Set("source", source)
		c.Next()
	}
}
...
func (h *Handler) ParseWxURL(c *gin.Context) {
	...
	tokenValue := c.GetString("token_value_masked")
	source := c.GetString("source")

	var req model.WxParseRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		h.writeLog(c, t0, label, tokenValue, source, "url", 1, "url is required", nil, gin.H{"url": ""})
	...
	if err != nil {
		h.writeLog(c, t0, label, tokenValue, source, "url", 1, err.Error(), nil, gin.H{"url": req.URL})
		c.JSON(http.StatusOK, model.WxParseResponse{Code: 1, Msg: err.Error()})
		return
	}
	h.writeLog(c, t0, label, tokenValue, source, "url", 0, "", data, gin.H{"url": req.URL})
...
func (h *Handler) ParseFinderFeedByObjectID(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		h.writeLog(c, t0, label, tokenValue, source, "finder", 1, "objectId and objectNonceId are required", nil, gin.H{})
	...
	if req.ObjectID == "" || req.ObjectNonceID == "" {
		h.writeLog(c, t0, label, tokenValue, source, "finder", 1, "objectId and objectNonceId are required", nil,
	...
	if err != nil {
		h.writeLog(c, t0, label, tokenValue, source, "finder", 1, err.Error(), nil,
			gin.H{"objectId": req.ObjectID, "objectNonceId": req.ObjectNonceID})
		c.JSON(http.StatusOK, model.WxParseResponse{Code: 1, Msg: err.Error()})
		return
	}
	h.writeLog(c, t0, label, tokenValue, source, "finder", 0, "", data,
		gin.H{"objectId": req.ObjectID, "objectNonceId": req.ObjectNonceID})
	c.JSON(http.StatusOK, model.WxParseResponse{Code: 0, Msg: "success", Data: data})
}
...
// tokenValue is the already-masked representation of the token the client
// sent — empty when the client sent no token at all.
func (h *Handler) writeLog(c *gin.Context, t0 time.Time, tokenLabel, tokenValue, source, kind string, status int, msg string, result, request any) {
	if h.storage == nil {
		return
	}
	t0Copy := t0
...
// maskToken returns a display-safe snapshot of the token: at most 8 leading
// chars, then "...". The full value is never persisted — the prefix is enough
// for an admin to identify which token was used (or attempted) without
// exposing the secret. Empty input → empty output.
